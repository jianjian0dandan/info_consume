...
class UserItem_search(Item):
    ...
    idstr = Field() # 字符串型用户ID, "2854532022"
    class_type = Field() # 1, ?
    ...
    RESP_ITER_KEYS = Field()
    PIPED_UPDATE_KEYS = Field()
    def __init__(self):
        """
        >>> a = UserItem()
        >>> a
        ...
        # utils.py中解析返回数据的字段
        self.setdefault('RESP_ITER_KEYS', ['id', 'name', 'class_type', 'gender', 'province', 'city', 'location', 'url', 'domain',
        ...
        self.setdefault('PIPED_UPDATE_KEYS', ['name', 'class_type', 'gender', 'province', 'city', 'location', 'url', 'domain',
                'geo_enabled', 'verified', 'verified_type', 'description',
                'followers_count', 'statuses_count', 'friends_count', 'favourites_count',
                'profile_image_url', 'allow_all_act_msg', 'created_at'])
        ...
        for key in default_empty_arr_keys:
            self.setdefault(key, [])
        '''
        if (search_type == 1): # searchBykw
        elif (search_type == 2): # 用户信息
        elif (search_type == 3): # 转发用户
        elif (search_type == 4): # 被转发用户
        elif (search_type == 5): # 评论用户
        elif (search_type == 6): # 评论被回复用户
        elif (search_type == 7): # 被评论用户
        elif (search_type == 8): # showBatch
        elif (search_type == 9): # userweibo
        elif (search_type == 10): # userweibo 被转发用户
        '''
...
class WeiboItem_search(Item):
    ...
    user = Field() # just uid, 用户uid
    mid = Field() # 16位微博ID, "3752470516005693"
    idstr = Field() # 16位微博ID, 字符串型微博ID, "3752470516005693"
    ...
    darwin_tags = Field() # [], ?
    pic_ids = Field() # 微博配图id, 多图时返回多图id, 无配图返回“[]”, 转发微博无法配图此字段为[]
    pic_urls = Field() # [{"thumbnail_pic": "http://ww4.sinaimg.cn/thumbnail/475b3d56gw1ek4vgg9x3xj20c60ee0t7.jpg"},]
    ...
    retweeted_status = Field() # just mid, 源微博mid
    ...
    # reply_comments = Field()
    # commented_status = Field()
    ...
    def __init__(self, search_type):
        ...
        for key in default_empty_arr_keys:
            self.setdefault(key, [])
        if (search_type == 1): # searchBykw
            self.setdefault('RESP_ITER_KEYS', ['created_at', 'id', 'mid', 'text', 'source', 'favorited', 'truncated', \
                    'in_reply_to_status_id', 'in_reply_to_user_id', 'in_reply_to_screen_name', \
                    'pic_urls', 'thumbnail_pic', 'geo', 'reposts_count', 'comments_count', \
                    'attitudes_count'])
            self.setdefault('PIPED_UPDATE_KEYS', ['favorited', 'truncated', 'reposts_count', 'comments_count', 'attitudes_count'])
            ...
            self.setdefault('RESP_ITER_KEYS', ['created_at', 'id', 'mid', 'text', 'source', 'pic_ids'])
            self.setdefault('PIPED_UPDATE_KEYS', [])
            ...
            self.setdefault('RESP_ITER_KEYS', ['created_at', 'id', 'mid', 'text', 'source', 'favorited', 'truncated', \
                    ...
                    'pic_urls', 'geo', 'reposts_count', 'comments_count', \
                    'attitudes_count'])
            self.sedefault('PIPED_UPDATE_KEYS', ['favorited', 'truncated', 'reposts_count', 'comments_count', 'attitudes_count'])
        elif (search_type == 8): # showBatch
            self.setdefault('RESP_ITER_KEYS', ['created_at', 'id', 'mid', 'text', 'source', 'favorited', 'truncated', \
                    'in_reply_to_status_id', 'in_reply_to_user_id', 'in_reply_to_screen_name', \
                    'pic_urls', 'geo', 'reposts_count', 'comments_count', \
            ...
            self.setdefault('PIPED_UPDATE_KEYS', ['favorited', 'truncated', 'reposts_count', 'comments_count', 'attitudes_count'])
        '''
        elif (search_type == 10): # userweibo 被转发微博
            self.setdefault('RESP_ITER_KEYS', ['created_at', 'id', 'mid', 'text', 'source', 'favorited', 'truncated', \
                    'in_reply_to_status_id', 'in_reply_to_user_id', 'in_reply_to_screen_name', \
                    'pic_urls', 'geo', 'reposts_count', 'comments_count', \
                    'attitudes_count'])
