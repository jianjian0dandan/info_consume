...
from utils import submit_identify_in, recommentation_in_auto, admin_delete_submit_in
...
def ajax_submit_identify_in():
    ...
    #input_data={'date': 2016-03-13, 'upload_data':[], user':submit_user, 'type':'', 'operation_type': 'show'/'submit'} type=uid/uname/url 
...
#write in version: 16-04-13
@mod.route('/admin_delete_in/')
def ajax_admin_delete_in():
    results = 0
    date = request.args.get('date', '')
    uid_string = request.args.get('uid_list', '')
    uid_list = uid_string.split(',')
    status = admin_delete_submit_in(date, uid_list)
