...
def save_detect_single_task(input_dict, task_max_count):
    results = {}
    ...
    #step2.5: identify the task 
    submit_user = task_information['submit_user']
    task_exist_count = identify_exist_detect_task_count(submit_user)
    if task_exist_count >= task_max_count:
        return 'more then limit'
    #step3: save to es
...
def save_detect_multi_task(input_dict, extend_mark, task_max_count):
    ...
    if extend_mark=='1':
        #print 'step3 save'
        #identify the task exist count is not more than task_max_count
        task_exist_count = identify_exist_detect_task_count(submit_user)
        if task_exist_count >= task_max_count:
        ...
        #identify the task exist count is not more than task_max_count
        task_exist_count = identify_exist_analysis_task_count(submit_user)
        if task_exist_count >= task_max_count:
            return 'more than limit'
...
def save_detect_attribute_task(input_dict, task_max_count):
    status = True
...
#input: input_dict {'task_information':{}, 'query_dict':{}}
#output: status True/False
def save_detect_event_task(input_dict, task_max_count):
    status = True
...
def detect2analysis(input_data):
    ...
    if len(uid_list) <= 1:
        return 'no enough user to analysis'
