...
topic_analysis_index.prototype = {   //获取数据，重新画表
  ...
  Draw_similar_topic:function(data){
    ...
    if(!data){
      ...
      html += '<thead><tr><th style="text-align: center;">话题名称</th><th style="text-align: center;">开始时间</th><th style="text-align: center;">终止时间</th><th style="text-align: center;">计算状态</th><th style="text-align: center;">操作</th></tr></thead>';
      html += '<tbody>';
  ...
  Draw_all_topic:function(data){
    ...
    html += '<center>';
    html += '<table class="table table-bordered" style="width:60%;margin-top:4%">';
    html += '<caption style="color: #00CC66;font-size: 28px;font-family: Microsoft YaHei;text-align:center;">我的话题&热门推荐 <a href="" onclick="window.location.reload()" style="float: right;font-size: 18px;margin: 12px 22px 0 0">'+'刷新'+'</a></caption>';
    html += '<thead><tr><th style="text-align: center;">任务类型</th><th style="text-align: center;">话题名称</th><th style="text-align: center;">开始时间</th><th style="text-align: center;">终止时间</th><th style="text-align: center;">计算状态</th><th style="text-align: center;">操作</th></tr></thead>';
    html += '<tbody>';
    ...
    for(key in data['own']){
      ...
      for(i=0;i<data['own'][key].length;i++){
        ...
        html += '<tr>';
        html += '<td style="text-align: center;">'+'我的创建'+'</td>';
        html += '<td style="text-align: center;">'+data['own'][key][i][0]+'</td>';
        ...
        html += '<td style="text-align: center;">'+start_time+'</td>';
        html += '<td style="text-align: center;">'+end_time+'</td>';
        html += '<td style="text-align: center;">'+compute_status+'</td>';
        var topic_name=data['own'][key][i][0];
        var en_name=key;
        var date_from = data['own'][key][i][1];
        var date_to=data['own'][key][i][2];
        html += '<td style="text-align: center;"><a href="" onclick="go_to_datail(\''+ topic_name +'\',\''+ en_name +'\',\''+ date_from +'\',\''+ date_to +'\',\''+ compute_status +'\')" target="_blank">'+'查看'+'</a> <a href="" onclick="commit_delete(\''+ en_name +'\',\''+ date_from +'\',\''+ date_to +'\',\''+ user_glo +'\')">'+'删除'+'</a></td>';
        html += '<tr>';
      }
    ...
    for(key in data['recommend']){
       for(i=0;i<data['recommend'][key].length;i++){
          html += '<tr>';
          html += '<td style="text-align: center;">'+'热门推荐'+'</td>';
          html += '<td style="text-align: center;">'+data['recommend'][key][i][0]+'</td>';
