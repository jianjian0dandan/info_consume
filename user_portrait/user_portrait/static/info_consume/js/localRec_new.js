...
function homepageinfo2() {
    //this.ajax_method='GET'; // body...
}
homepageinfo2.prototype= {
...
var place2=new homepageinfo2();
// var uid = 1640601392;
function nums() {
    var url = "/attribute/localRec/?uid=" + uid;
    place2.call_request(url,territory);
}
nums();
function Draw_weibo_table(data){
    ...
    if(PageNo.value!="")                                       //判断每页显示是否为空
    {
        ...
        oTBody.setAttribute('class','In-table2');                   //定义class
    ...
    Nep.onclick=function()
    {
        if(currentPage<SumPage)                                 //判断当前页数小于总页数
        {
            ...
            oTBody.setAttribute('class','In-table2');
            InTb.appendChild(oTBody);
            var a;                                                 //定义变量a
