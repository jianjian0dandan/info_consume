...
function detect_task_status(data) {
    //console.log('detacetsdfa',data);
    var html = '';
    
    $('#detect_task_status').empty();
    html += '<div><span style="float: left;margin-left: 778px;margin-bottom: 10px;cursor:pointer;" type="button"data-toggle="modal" data-target="#detect_search_modal" ><u>任务搜索</u></span>';
    html += '<span id="show_all_task" style="float: left;margin-left: 20px;margin-bottom: 10px;cursor:pointer;"><u>显示全部任务</u></span>';
    html += '</div><br><div id="task_table_2" style="margin-left:30px;"></div>';
    $('#detect_task_status').append(html);
    if (data != ''){
        var sort_scope = data.sort_scope;
        $('#task_table_2').empty();
        var html = '';
        html += '<br><table id="task_table" class="table table-bordered table-striped table-condensed datatable" style="width:900px;">';
        html += '<thead>';
        ...
        for(var i=0;i<data.length;i++){
            ...
            var delete_this = '<span class="de_delete_this"><b><u class="" style="cursor:pointer;">删除</u></b></span>';
            if(data[i][5] == 0){
                var status = '正在计算';
            }else{
        ...
        html += '</table>';
        $('#task_table_2').append(html);
    }else{
        $('#task_table_2').empty();
        var html = '<div style="text-align: center;background-color: #cccccc;width: 900px;">暂无相关任务</div>'
        $('#task_table_2').append(html);
    }
}
...
function draw_user_in_table(data){
    ...
    if(data.length == 0){
        ...
        for(var i=0;i<data.length;i++){
            var user_link = '/index/personal/?uid=' + data[i][0];
            html += '<tr>';
            html += '<td style="text-align:center;"><a href='+user_link+' target="_blank">'+data[i][0]+'</a></td>';
            var name = data[i][1][0];
            if(data[i][1][0] == 'unknown'||data[i][1][0] == ''){
                name = data[i][0];
            }
...
function draw_user_out_table(data){
    ...
    if(data.length == 0){
        ...
        for(var i=0;i<data.length;i++){
            var user_link = '/index/personal_out/?uid=' + data[i][0];
            html += '<tr>';
            html += '<td style="text-align:center;"><a href='+ user_link +' target="_blank">'+data[i][0]+'</a></td>';
            var name = data[i][1][0];
            if(data[i][1][0] == 'unknown'||data[i][1][0] == ''){
        ...
        html += '</table>';
        html += '<p><span onclick="recom_in()" class="portrait_button" style="float:right;margin-right: 10px;cursor:pointer;width: 60px;height: 25px;text-align: center;line-height: 25px;">推荐入库</span></p>'
...
function Draw_keyword(data){
  ...
  if(data.length == 0){
      ...
      for (var i = 0; i < data.length; i++) {
         var s = i.toString();
         var m = i + 1;
         html += '<tr style=""><th style="text-align:center">' + m + '</th><th style="text-align:center"><a href="/index/search_result/?stype=2&uid=&uname=&location=&hashtag=&adkeyword=' + data[i][0] +  '&psycho_status=&domain&topic" target="_blank">' + data[i][0] +  '</a></th><th style="text-align:center">' + data[i][1].toFixed(0) + '</th></tr>';
      };
      html += '</table>'; 
  ...
  //console.log(div_name);
    if(data.length == 0){
      ...
      for(var i=0;i<data.length;i++){
        ...
        html += '<span class="comment_count">言论敏感度(' + sensitive_score + ')</span></div>';
        html += '<div class="m">';
        html += '<u>' + date + '</u>&nbsp;-&nbsp;';
...
function show_all_related_weibo(data) {
    ...
    html += '<input type="radio" name="sort_radio_weibo" id="retweet" value = "retweet" checked="checked" style="margin-left: 30px;"> 转发数';
    html += '<input type="radio" name="sort_radio_weibo" id="comment" value = "comment" style="margin-left: 30px;"> 评论数';
    html += '<input type="radio" name="sort_radio_weibo" id="sensitive" value = "sensitive" style="margin-left: 30px;">言论敏感度';
    html += '<input type="radio" name="sort_radio_weibo" id="timestamp" value = "timestamp"  style="margin-left: 30px;"> 时间';
...
function show_detail_click(data,sort_type){
    ...
    //$('#nodata_message').css('display','none');
...
function show_detail(data){
    ...
    if(data.keywords.length == 0 && data.weibo.length == 0 && data.in_portrait_result == 0){
        $('#nodata_message').css('display', 'block');
...
function Draw_detect_charts_percent(flag, data){
    //console.log(data);
    if(data == ''){
        $('#result_detect_charts_percent').empty();
        $('#result_detect_charts_percent').append('<h4 style="margin-left: 30px;width: 900px;text-align:center;min-height: 100px;background-color: #cccccc;line-height: 100px;">暂无数据</h4>');
    }else{
        var data_x_ = [];
        var data_y_1 = [];
        var data_y_0 = [];
        ...
        for(var i=0;i<data["7"].length;i++){
            var total_num = data["1"][i][1]+data["7"][i][1]+data["0"][i][1];
            total.push(total_num);
        }
        //console.log(total);
        for(var i=0;i<data["1"].length;i++){
            ...
            if(total[i] == 0){
                ...
                data_y_1.push((data["1"][i][1]/total[i]).toFixed(2));
            }
        }
        for(var i=0;i<data["7"].length;i++){
            if(total[i] == 0){
                ...
                data_y_7.push((data["7"][i][1]/total[i]).toFixed(2));
            }
        }
        for(var i=0;i<data["0"].length;i++){
            if(total[i] == 0){
                ...
                data_y_0.push((data["0"][i][1]/total[i]).toFixed(2));
            }
        }
        if(data["1"].length < 100){
            ...
            var zoom = true;
            var zoom_start = 100 - parseInt(100/data["1"].length*100);
        }
        //var zoom_start = data["1"].length/20
        var myChart = echarts.init(document.getElementById('result_detect_charts_percent')); 
        var option = { 
            title : {
                text  :'',
                x: 'center',
        ...
        require([
          ...
          function(ec){
              ...
              function eConsole(param) {
                  ...
                  global_url = detail_url;
                  detail_url += '&sort_type=retweet';  //默认时间戳排序
                  console.log(detail_url);
                  call_sync_ajax_request(detail_url, show_detail);
                  //control();
              }
...
$('#detect_choose').change(function(){
    ...
    if($('#detect_choose').val() == 'all_limit_keyword') {
        var html = '';
        html += '<input id="keyword_detect" type="text" class="form-control" style="width:275px;height:25px;" placeholder="请输入关键词，多个词用空格分开">';
...
function submit_detect(){
    ...
    if(keyword == ''){  //检查输入词是否为空
        ...
        if(keyword == undefined){  //没有输入的时候，更新图表
            ...
            $('#detect_task_status').css('display', 'block');
            ...
            var keyword_string = [];
            var keyword0 = keyword.split(/\s+/g);
            for (var i =0;i<keyword0.length;i++){
                if (keyword0[i].length>0){
                     keyword_string.push(keyword0[i]);
                }
            }
            var url = '/sentiment/submit_sentiment_all_keywords/?start_date='+time_from+'&end_date='+time_to+'&keywords='+keyword_string +'&submit_user=' + username +'&segment='+sort_norm +'&task_max_count='+moodnum;
...
function search_task(){
   // window.clearInterval(flag_fresh);
    //window.clearInterval(all_fresh);
    ...
    if(submit_key != ''){
        var keyword_string = submit_key.split(' ').join(',');
        search_url += '&keywords='+ keyword_string;
    }
...
$('.de_delete_this').live('click',function(){
    ...
    if (a == true){
        ...
        var ts0 = $(this).parent().prev().prev().text();
        var keys = $(this).parent().prev().prev().prev().prev().prev().text();
        var ts=Date.parse(new Date(ts0))/1000;
        //var id= $(this).prev().text();
        var del_url = '/sentiment/delete_sentiment_all_keywords_task/?submit_ts='+ts+'&submit_user='+username+'&keywords='+keys;
...
$('#show_all_task').live('click', function(){
    //all_fresh = window.setInterval(draw_table_fresh, 3000)
    //console.log(all_fresh);
...
draw_table_fresh();
var all_fresh = -10;
var flag_fresh = window.setInterval(draw_table_fresh, 60000);
//console.log(flag_fresh)

var url = '/sentiment/sentiment_all_portrait/?start_date='+time_from+'&end_date='+time_to+'&segment='+$('#sort_select_2 option:selected').val();
//var url = '/sentiment/sentiment_all/?start_date='+time_from+'&end_date='+time_to+'&segment='+$('#sort_select_2 option:selected').val();
call_sync_ajax_request(url,Draw_in_all_detect_charts);
...
var show_time = date +' '+  hh.toString() + ':' + show_mm.toString();
show_time = show_time.replaceAll('/', '-');
//console.log(show_time);
var recent_time = Date.parse(show_time)/1000;
//console.log(recent_time);
var detail_url = '/sentiment/sentiment_weibo_keywords_user/?start_ts='+recent_time+'&task_type=in-all&segment=fifteen&sentiment=7';
global_url = detail_url;
detail_url += '&sort_type=retweet';
console.log(detail_url);
init_control();
$('#click_time').append(show_time);
$('#click_time').append(' 至 终止日期')
