...
function g_info(data){
	...
	if(data.topic.length == 0){
        ...
        //console.log("topic");
        Draw_topic_group_spread_1(data.topic,'topic', 'topicWordList','showmore_topic','领域','人次');
    };
    ...
    if(data.domain.length == 0){
        ...
        //console.log('domain');
        Draw_topic_group_spread_1(data.domain,'domain', 'domainWordList','showmore_domain','身份','人数');
        //console.log('sdfdsf');
    }
}
...
var b_user = $('#g_useremail').text();
var basic_url='/group/show_group_result/?task_name='+name+'&submit_user='+b_user+'&module=basic';
...
$('#group_basic_weibo').css('display', 'none');
$('#loadingHTML').css('display','block');
var url = "/group/group_user_weibo/?task_name="+name+"&submit_user="+b_user+"&sort_type=retweeted";
...
function bind_basic_click(){
	$('[name="mode_choose"]').change(function(){
       //$('group_basic_weibo').empty();
       //$('group_basic_weibo').append('ddddddddddddddd')
        // var url = "group/group_user_weibo/?task_name="+name+"&submit_user=admin&sort_type="+$(this).val();
        $('#group_basic_weibo').css('display', 'none');
        $('#loadingHTML').css('display','block');
        var url = "/group/group_user_weibo/?task_name="+name+"&submit_user="+b_user+"&sort_type="+ $('input[name="mode_choose"]:checked').val();
        //console.log(url);
        g_basic.call_sync_ajax_request(url, g_basic.ajax_method,weiboData);
	});
}
...
function page_group_basic_weibo(start_row,end_row,data, sub_div_name){
    ...
    for (var i = start_row; i < end_row; i += 1){
        ...
        var sensitive_count = data[s][10];
        if(data[s][4] == null){
            var location = '未知'
        }else{
            var location = data[s][4].split('&').join(' ');
        }
        var date = data[s][7];
        ...
        if (i%2 ==0){
            ...
            html += '<p style="margin-left:10px;">昵称：<a target="_blank" href="/index/personal/?uid=' + uid + '">' + uname + '</a> ('+location+')&nbsp;&nbsp;发布:<font color=black>' + text + '</font></p>';
//            html += '<p style="margin-top:-5px;margin-left:10px;">'+date+'-<a color:#e0e0e0 target="_blank" style="" href="'+tweet_ulr+'">微博</a>-<a target="_blank" href="/index/personal_detail/?uid=' + uid + '">用户详情</a>-<a>转发树</a>';
//             html += '<span style="margin-left:300px;">转发数（'+ retweet_count +'）&nbsp&nbsp|&nbsp&nbsp评论数（'+ comment_count +'）&nbsp&nbsp|&nbsp&nbsp言论敏感度（'+ sensitive_count+'）</span></p>';
             
            html += '<p style="width:100;margin-top:-5px;margin-left:10px;"><span style="width:65%;float:left"><u>'+date+'</u> - <a color:#e0e0e0 target="_blank"  href="'+tweet_ulr+'">' + '微博' + '</a>-<a target="_blank" href="/index/personal_detail/?uid=' + uid + '">用户</a></span>';
            html += '<span style="width:35%;color:#858585"><span>转发数('+ retweet_count +')&nbsp;&nbsp;|&nbsp;&nbsp;</span><span>评论数('+ comment_count +')</span>&nbsp;&nbsp;|&nbsp;&nbsp;<span>言论敏感度('+sensitive_count+')</span></p>';
           // html += '<p style="margin-top:-5px;margin-left:10px;">'+date+'-<a color:#e0e0e0 target="_blank" style="" href="'+tweet_ulr+'">微博</a>-<a target="_blank" href="/index/personal_detail/?uid=' + uid + '">用户详情</a>-<a>转发树</a>';
            // html += '<span style="margin-left:300px;">转发数（'+ retweet_count +'）&nbsp&nbsp|&nbsp&nbsp评论数（'+ comment_count +'）&nbsp&nbsp|&nbsp&nbsp敏感数（'+ sensitive_count+'）</span></p>';
    ...
    $('#group_basic_weibo').css('display', 'block');
    $('#loadingHTML').css('display','none');
}

function Draw_topic_group_spread_1(data, radar_div, motal_div, show_more,title_0,title_1){
  var topic = [];
  var html = '';
    if(data[0][1] == 0){
      $('#'+ motal_div).empty();
  ...
  if(topic_sta.length == 0){
      ...
      html += '<tr><th style="text-align:center">排名</th><th style="text-align:center">'+title_0+'</th><th style="text-align:center">'+title_1+'</th></tr>';
    ...
    var topic_result = [];
    topic_result = get_radar_data_1(topic_val);
...
function get_radar_data_1 (data) {
  var topic = data;
  var topic_name = [];
  var topic_value = [];
  ...
  for(var i=0;i<max_topic;i++){ //设置最大值的话题的阈值
    ...
    name_dict["max"]=parseInt(topic_value);
