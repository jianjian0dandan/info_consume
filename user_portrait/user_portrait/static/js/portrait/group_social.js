...
function draw_relation_net(data,name,symbols){
    ...
    if(data[name].length==0){
        $('#relation_net').empty();
        var html = '<span style="margin-left:450px;line-height:70px;">暂无数据</span>';
        $('#relation_net').css('height',200);
        $('#in_tweet').css('height',200);
        $('#relation_net').append(html);
        $('#group_in_table').css('display','none');
    }else{
    for (var i=0;i<data[name].length;i++){
        ...
        var content1 = {};
        content1['category'] = 1;
        if(data[name][s]['3']=='unknown'||data[name][s]['3']==''){
            content1['name'] = data[name][s][0]+'\n'+'('+data[name][s]['0']+')';
            content1['label'] = data[name][s][0];
        }else{
        ...
        content1['symbolSize'] = [15, 15];
        total_content.push(content1);
        var content2 = {};
        content2['category'] = 1;
        if(data[name][s]['4']=='unknown'||data[name][s]['4']==''){
            content2['name'] = data[name][s][1]+'\n('+data[name][s]['1']+')';
            content2['label'] = data[name][s][1];
        }else{
...
function draw_relation_out_net(data,name,symbols){
    ...
    if(data[name].length==0){
        $('#relation_out_net').empty();
        var html = '<span style="margin-left:450;line-height:70px;">暂无数据</span>';
        $('#relation_out_net').append(html);
        $('#out_tweet').css('height',200);
        $('#group_out_table').css('display','none');
    }else{
    var source_content = []
    for (var i=0;i<data[name].length;i++){
        var s=i.toString();
        var content1 = {};
        content1['category'] = 1;
        if(data[name][s]['5']=='unknown'||data[name][s]['5']==''){
            content1['name'] = data[name][s][1]+'\n'+'('+data[name][s]['1']+')';
            content1['label'] = data[name][s][1];
        }else{
...
function draw_in_table(data){
    ...
    s =i.toString();
    //console.log(data['social_in_record'][s]);
    var uname1 = data['social_in_record'][s]['4'];
    var uname2 = data['social_in_record'][s]['3'];
    if (uname1 =='unknown'||uname1 ==''){
        uname1 = data['social_in_record'][s][1];
    }
    if (uname2 =='unknown'||uname2 ==''){
        uname2 = data['social_in_record'][s][0];
    }
...
function draw_out_table(data){
    ...
    for (var i=0;i<data['social_out_record'].length;i++){
        s =i.toString();
            ...
            if (uname1 =='unknown'||uname1 ==''){
                uname1 = data['social_out_record'][s][0];
            }
            if (uname2 =='unknown'||uname2 ==''){
                uname2 = data['social_out_record'][s][1];
            }
...
function g_soc_load(){
var so_user = $('#g_useremail').text();
var social_url = '/group/show_group_result/?task_name='+name+'&module=social'+'&submit_user='+so_user;
