...
Search_weibo_recommend.prototype = {
  ...
  Re_Draw_Out_table: function(data){
    ...
    var user_url ;
    html = '';
    html += '<table id="recommend_out_table_new" style="width:100%;word-wrap: break-word;word-break: break-all;table-layout:fixed;" class="table table-striped table-bordered bootstrap-datatable datatable responsive">';
    html += '<thead><th width="55" style="60px;">用户ID</th>';
    html += '<th width="60">昵称</th>'
    html += '<th width="70">领域</th>'
    html += '<th width="150">话题</th>'
    html += '<th width="50">活跃度</th>'
    html += '<th width="60">身份敏感度</th>'
    html += '<th width="50">影响力</th>';
    html += '<th width="60">言论敏感度</th>';
    //html += '<th style="">网民详情</th>'
   // html += '<th style="">推荐详情</th>'
   // html += '<th style="width:60px;">删除</th>'
    html += '<th width="10" class="dropwidth" style="width:15px !important;"><input name="recommend_out_all" id="recommend_out_all" type="checkbox" value="" onclick="recommend_out_all()" /></th>';
    html += '</thead>';
    //html += '<thead><tr><th style="width:100px;">用户ID</th><th>昵称</th><th>领域</th><th>话题</th><th style="width:">活跃度</th><th style="width:100px">重要度</th><th style="width:100px">影响力</th><th>' + '<input name="recommend_out_all" id="recommend_out_all" type="checkbox" value="" onclick="recommend_out_all()" />' + '</th></tr></thead>';
    var item = data;
    //console.log(item);
    html += '<tbody>';
    for(var i in item){
      ...
      html += '<td class="center">'+ item[i][9] +'</td>';
      html += '<td class="center">'+ item[i][7] +'</td>';
      html += '<td class="center">'+ item[i][8] +'</td>';
      html += '<td class="center">'+ item[i][10] +'</td>';
      html += '<td class="center"><input name="out_status" class="out_status" type="checkbox" value="' + item[i][0] + '" /></td>';
      html += '</tr>';
    }
    html += '</tbody>';
    html += '</table>';
    ...
    $('#recommend_out_table_new thead th').removeAttr("style");
          ...
          $('#recommend_load').css('display','none');
          $('#recommend_out').css('display','block');
  ...
  Re_Draw_table: function(data){
    ...
    html = '';
    html += '<table id="recommend_table_new" style="width:100%;table-layout:fixed" class="table table-striped table-bordered bootstrap-datatable datatable responsive">';
    //html += '<table id="recommend_table_new" >';
    html += '<thead><th width="70">用户ID</th>';
    html += '<th width="70">昵称</th>'
    //html += '<th style="width:80px">注册地</th>'
    html += '<th width="57">粉丝数</th>'
    html += '<th width="58">微博数</th>'
    html += '<th width="57">影响力</th>'
    html += '<th width="68">言论敏感度</th>'
    html += '<th width="100">敏感词</th>'
    html += '<th width="60">网民详情</th>'
    html += '<th width="70">推荐人数</th>'
    html += '<th width="80">业务人员推荐</th>'
    html += '<th width="40">删除</th>'
    html += '<th width="15" style="padding:3px;"><input name="recommend_all" id="recommend_all" type="checkbox" value="" onclick="recommend_all()" />';
    html += '</th></thead>';
    var item = data;
    //console.log('fdgv', item[0]);
    html += '<tbody>';
    for(var i in item){
      item[i] = replace_space(item[i]);
      recommends_list[item[i][0]]=[item[i][9], item[i][10]];
      //recommends_list[item[i][0]].push(item[i][10]);
      if(item[i][1] == '未知'){
        var name= item[i][0]
      }else{
        var name = item[i][1]
      ...
      var flag_rec = '是'
      if(item[i][9].length == 1 && item[i][9][0]=='system'){
        flag_rec = '否'
      }
      ...
      html += '<tr>';
      html += '<td class="center"><a href='+ user_url+ ' target="_blank">'+ item[i][0] +'</td>';
      html += '<td class="center" style="width:100px;word-wrap:break-word">'+ name +'</td>';
      //html += '<td class="center">'+ item[i][2] +'</td>';
      html += '<td class="center">'+ item[i][3] +'</td>';
      html += '<td class="center">'+ item[i][4] +'</td>';
      html += '<td class="center">'+ item[i][5] +'</td>';
      html += '<td class="center">'+ item[i][7].toFixed(2) +'</td>';
      html += '<td class="center">'+ item[i][6] +'</td>';
      html += '<td class="center"><a style="cursor:pointer;" name="details" id="'+ item[i][0] +'" title="'+ item[i][1] +'">网民详情</a></td>';
      html += '<td class="center"><a style="cursor:pointer;" name="recommends" id="'+ item[i][0] +'" title="'+ item[i][1] +'">'+item[i][9].length+'</a></td>';
      html += '<td class="center">'+ flag_rec +'</td>';
      html += '<td class="center"><a style="cursor:pointer;" name="remove" id="'+item[i][0]+'"title="'+item[i][1]+'">删除</a></td>';
      html += '<td class="center" style="padding:3px;"><input name="in_status" class="in_status" type="checkbox" value="' + item[i][0] + '" /></td>';
      html += '</tr>';
    }
    html += '</tbody>';
    html += '</table>';
    ...
    $('[name="remove"]').click(function(){
        var del_uid = $(this).attr('id');
        var show_date = $('#recommend_date_select option:selected').val();
        var del_url = '/recommentation/admin_delete_in/?uid='+del_uid+'&date='+show_date;;
        $.ajax({
            url: del_url,
            type: 'GET',
            dataType: 'json',
    ...
    $('[name="details"]').click(function(){
      ...
      function show_details(data){
        ...
        if(data['hashtag'].length==0){
          ...
          hashtag_html += '<thead><tr><th style="text-align:center;vertical-align:middle;width:80px">排名</th>';
          hashtag_html += '<th style="text-align:center;vertical-align:middle;width:200px">微话题</th><th style="text-align:center;vertical-align:middle;width:80px">微博数</th></tr></thead>';
          hashtag_html += '<tbody>';
    ...
    $('[name="recommends"]').click(function(){
      ...
      var user_list = recommends_list[detail_uid];
        ...
        $('#user_list').empty();
          $('#user_list').html(user_list[0].join(','));
          $('#recommend_count').html(user_list[0].length);
    ...
    $('#recommend_table_new').dataTable({
       ...
       // "sPaginationType": "boostrap",
        //"aoColumnDefs":[{"sWidth":"5em",  "aTargets":[0]},{"sWidth":"6em",  "aTargets":[1]},{"sWidth":"6em",  "aTargets":[2]},{"sWidth":"4.5em",  "aTargets":[3]},{"sWidth":"5.5em",  "aTargets":[4]},{"sWidth":"4.5em",  "aTargets":[5]},{"sWidth":"4.5em",  "aTargets":[6]},{"sWidth":"10em",  "aTargets":[7]},{"sWidth":"6em",  "aTargets":[8]}, {"bSortable": false, "aTargets":[7,10,11]}],
        "aoColumnDefs":[{"bSortable": false, "aTargets":[7,10,11]}],
    ...
    // page control end
          $('#recommend_load').css('display','none');
          $('#recommend_in_tab').css('display','block');
  }
}
...
Search_weibo_history.prototype = {
  ...
  Re_Draw_table: function(data){
    ...
    html += '<thead><tr style="height:40px;"><th style="width:140px">用户ID</th><th style="width:120px;">昵称</th><th>注册地</th><th style="width:100px">粉丝数</th><th style="width:100px">微博数</th><th style="width:100px">影响力</th><th>入库日期</th><th>计算状态</th></tr></thead>';
    var item = data;
    html += '<tbody>';
    for(var i in item){
      item[i] = replace_space(item[i]);
      ...
      $('#recommend_in_button').click(function(){
          ...
          if(recommend_uids.length == 0){
            ...
            if($('input[name="instant"]:checked').val()==1){
              ...
              if(sure==true){
                  $('#recommend_load').css('display','block');
                  $('#recommend_in_tab').css('display','none');
                  ...
                  draw_table_recommend_new = new Search_weibo_recommend(url_recommend_new, '#recommend_in_tab');
...
function outbindOption(){
  $('.re_in_out').click(function(){
     var select_tab=$(this).text();
     if(select_tab=='入库历史'){
     ...
     }else if(select_tab=='出库管理'){
         if(out_flag==false){
          $('#recommend_load').css('display','block');
          $('#recommend_out').css('display','none');
  ...
  $('#no_recommend_button').click(function(){
      ...
      $('#recommend_out').empty();
      //var waiting_html = '<div style="text-align:center;vertical-align:middle;height:40px">数据正在加载中，请稍后...</div>';
      //$('#recommend_out').append(waiting_html);
          $('#recommend_load').css('display','block');
          $('#recommend_out').css('display','none');
      var url_recommend_new = '/recommentation/show_out/?fields=uid,uname,domain,topic,influence,importance,activeness';
...
draw_table_recommend = new Search_weibo_recommend(url_recommend, '#recommend_in_tab');
