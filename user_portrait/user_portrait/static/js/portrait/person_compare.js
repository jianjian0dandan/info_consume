...
function Compare(url_photo, portrait, tag_data){
    ...
    html += '</tr>';
    j = 0;
    html += '<tr class="list-2"><td class="cate_title" style="width:90px;text-align:right">身份敏感度</td>';
    for(var k in portrait){
        j += 1;
    ...
    html += '</tr>';
    j = 0;
    html += '<tr class="list-2"><td class="cate_title" style="width:90px;text-align:right">言论敏感度</td>';
    for(var k in portrait){
        j += 1;
    ...
    html += '</tr>';
    j = 0;
    html += '<tr><td colspan="'+ (num+1) +'" name="list-3" class="cate_title" style="font-size:20px"><b>身份特征</b></td></tr>';
        ...
        if(portrait[k]['tag'].length == 0){
        	...
        	for(var i = 0; i < portrait[k]['tag'].length; i++){
        		if(i == portrait[k]['tag'].length -1){
        			html += '<span class="input-group-addon" style="width:auto;border:1px solid white; border-radius: 8px;display:inline-block">'+ portrait[k]['tag'][i].replace('-',':') +'</span>';
        		}
        		else{
        			html += '<span class="input-group-addon" style="width:auto;border:1px solid white; border-radius: 8px;display:inline-block">'+ portrait[k]['tag'][i].replace('-',':') +'</span>';
        		}
        ...
        if(portrait[k]['tag'].length == 0){
            ...
            for(var i = 0; i < portrait[k]['group_tag'].length; i++){
                if(i == portrait[k]['group_tag'].length -1){
                    html += '<span class="input-group-addon" style="width:auto;border:1px solid white; border-radius: 8px;display:inline-block">'+ portrait[k]['group_tag'][i].replace('-',':') +'</span>';
                }
                else{
                    html += '<span class="input-group-addon" style="width:auto;border:1px solid white; border-radius: 8px;display:inline-block">'+ portrait[k]['group_tag'][i].replace('-',':') +',</span>';
                }
...
//console.log(portrait[key]['sensitive_words'],portrait[key]['sensitive_words'].length)
        if(portrait[key]['sensitive_words'].length == 0 || (portrait[key]['sensitive_words'].length == 1 && portrait[key]['sensitive_words'][0][0] == 'm')){
            $('#'+div).empty();
            $('#'+div).append('<span style="display:block; padding-top:83px">该数据为空</span>');
        }else{
