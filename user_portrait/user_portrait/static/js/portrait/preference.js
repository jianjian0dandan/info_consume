  ...
  if(data[0][1].toFixed(3) == 0){
      ...
      if(topic_sta.length == 0){
         //console.log(div_name);
          html = '<h3 style="font-size:20px;text-align:center;margin-top:40%;">暂无数据</h3>';
          ...
          for (var i = 0; i < topic_sta.length; i++) {
             var s = i.toString();
             var m = i + 1;
             html += '<tr style=""><th style="text-align:center">' + m + '</th>';
             //html += '<th style="text-align:center"><a href="/index/search_result/?stype=2&uid=&uname=&location=&hashtag=&adkeyword=' + topic_name_sta[i] +  '&psycho_status=&domain&topic" target="_blank">' + topic_name_sta[i] +  '</a></th>';
             html += '<th style="text-align:center">' + topic_name_sta[i] +  '</th>';
             html += '<th style="text-align:center">' + topic_sta[i].toFixed(3) + '</th></tr>';
          };
          html += '</table>'; 
...
function show_domain0(data, is_school, school_name){
  if(is_school == '0'){
      school_name = '\n非校园网用户'
  }else{
      school_name = '校园网用户\n' + school_name.split('&').join(' ');
  }
  // var html = '';
  //html += '<h3>用户领域分析</h3>';
  data1 = '根据注册信息分类：\n'+data[0][0];
    ...
    series : [
        {
            ...
            rootLocation: {x: 220, y: 'center'}, // 根节点位置  {x: 'center',y: 10}
            nodePadding: 50,
            symbol: 'circle',
            symbolSize: 30,
            itemStyle: {
            ...
            data: [
                {name:data4 ,itemStyle:{normal:{label:{position: 'inside',textStyle:{fontWeight:'bold',fontSize:14}}}},
		children:[{name:data1},{name:data2},{name:data3},{name:data5}]}
            ]
        }
    ]
};
...
function show_results0(data){
  ...
  var school_name = data.results.school_string;
  var is_school = data.results.is_school;
  show_domain0(domain, is_school, school_name);
}
