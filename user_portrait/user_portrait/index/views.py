...
@mod.route('/nouns_explain/')
@login_required
def nouns_explain():
    return render_template('portrait/nouns_explain.html')
...
@mod.route('/sensitive_manage/')
@login_required
@roles_required("administration")
def sensitive_manage():
    return render_template('portrait/sensitive_manage.html')
...
@roles_required("group")
def group_identify():
    return render_template('portrait/group_identify_new.html')
...
@mod.route('/personal_out/')
@login_required
@roles_required("profile")
def personal_out():
    uid = request.args.get('uid', '1642591402')

    return render_template('portrait/personal_out.html', uid=uid)
...
@roles_required("administration")
def logmanage():
    #return redirect("http://114.255.183.77:8080/")
    return redirect("http://114.255.183.77:8080/#/discover?_a=(columns:!(_source),index:%5Bfilebeat-%5DYYYY.MM.DD,interval:auto,query:(query_string:(analyze_wildcard:!t,query:'*')),sort:!('@timestamp',desc))&_g=(refreshInterval:(display:'30%20minutes',pause:!f,section:2,value:1800000),time:(from:now%2Fd,mode:quick,to:now%2Fd))")
